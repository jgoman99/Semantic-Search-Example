...
class Embedding_Cache:
    def __init__(self, embedding_cache_path = "../Data/embeddings_cache.jsonl"):
    ...
    def set_embedding_cache_path(self, path):
        self.embedding_cache_path = Path(path)
...
# semantic_search
def semantic_search(search_query : [str], text_list : [str],\
    ...
    if type(search_query) != list:
        raise TypeError("search_query must be a list")

    # combine items in list to avoid caching twice
    text_list.extend(search_query)
    text_df = embed_string_list(text_list)
    
    string_embeddings = text_df['vector'].iloc[0:-1].tolist()
    search_query_embedding = text_df['vector'].iloc[-1]
    
    score_list = [cosine_similarity(item,search_query_embedding) for item in string_embeddings]
    indices = np.argsort(score_list)[::-1]
    
    dict_list = [{'text':text_list[index],'order':index,'score':score_list[index]} for index in indices]
